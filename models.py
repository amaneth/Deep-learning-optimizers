...
class NeuralNetwork:
    ...
    def backprop(self, x,y,epoch,iteration):
        ...
        d_weights1 = -np.dot(x.T,  (np.dot(2*(y-self.output) * self.sigmoid_derivative(self.output), self.weights2.T)\
              * self.sigmoid_derivative(self.layer1)))

        d_bias2 = -(2*(y-self.output ) * self.sigmoid_derivative(self.output))
        d_bias1 = -(np.dot(2*(y-self.output) * self.sigmoid_derivative(self.output), self.weights2.T)\
              * self.sigmoid_derivative(self.layer1))
        ...
        if self.optim=='adam':
            ...
            raise NameError(f'Invalid optimizer name, no optimizer named {self.optim} found.')
